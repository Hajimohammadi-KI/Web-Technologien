...
d3.json("http://localhost:3000/wetterdaten/temperatur")
  .then((data) => {
   ...
   // Create SVG container inside the #piechart div
const svg = d3.select("#piechart") 
  .append("svg") // Add an SVG element
  .attr("viewBox", `0 0 ${width} ${height}`) // Make the SVG responsive using viewBox
  .attr("preserveAspectRatio", "xMidYMid meet") // Keep the aspect ratio when resizing
  .classed("w-100", true) // Use full width inside its container (Bootstrap class)
  .append("g") // Add a <g> group inside the SVG to hold the chart
  .attr("transform", `translate(${width / 2}, ${height / 2})`); // Move the group to the center of the SVG
